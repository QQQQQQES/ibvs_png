...
public:
	...
	ros::Publisher thrust_model_pub; // publish thr2acc for other controllers
	...
	// PNG guidance data (AL controller outputs)
	...
	bool cmd2_is_received(const ros::Time &now_time);
	...
	// PNG guidance callbacks (public for subscription)
	void update_al_angular_velocity(const geometry_msgs::Vector3::ConstPtr& msg);
...
private:
	...
	bool png_guidance_active_ = false;
	ros::Time last_desired_vel_dir_time_;
	// ---- control related ----
	Desired_State_t get_hover_des();
	Desired_State_t get_cmd_des();

	// ---- PNG guidance ----
	void check_png_guidance_state();
