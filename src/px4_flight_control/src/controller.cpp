    ...
    Controller_Output_t &u)
{
  /* WRITE YOUR CODE HERE */
      ...
      recordThrustForEstimation(u);

      return debug_msg_;
}

void LinearControl::recordThrustForEstimation(const Controller_Output_t &u)
{
  // Only record valid thrust values to prevent numerical instability
  if (std::isfinite(u.thrust) && u.thrust > 0.0 && u.thrust < 1.0)
  {
    ...
    const Parameter_t &param)
{
  ...
  while (timed_thrust_.size() >= 1)
  {
    ...
    // Validate inputs before RLS update
    if (!std::isfinite(thr) || !std::isfinite(est_a(2)) || thr <= 0.0 || thr >= 1.0)
    {
      timed_thrust_.pop();  // Pop invalid data
      continue;
    }

    timed_thrust_.pop();  // Pop valid data before using
    ...
    // Clamp thr2acc to reasonable range [5.0, 50.0]
    // thr2acc_ = std::max(5.0, std::min(50.0, thr2acc_));
