...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber odom_sub =
        nh.subscribe<nav_msgs::Odometry>("/iris_0/mavros/local_position/odom",
                                         100,
    ...
    ros::Subscriber imu_sub =
        nh.subscribe<sensor_msgs::Imu>("/iris_0/mavros/imu/data", // Note: do NOT change it to /mavros/imu/data_raw !!!
                                       100,
    ...
    ros::Subscriber al_angular_vel_sub =
        nh.subscribe<geometry_msgs::Vector3>("/al_controller/angular_velocity",
                                             10,
                                             boost::bind(&PX4CtrlFSM::update_al_angular_velocity, &fsm, _1));

    ros::Subscriber al_thrust_val_sub =
        nh.subscribe<geometry_msgs::Vector3>("/al_controller/thrust_value",
                                             10,
                                             boost::bind(&PX4CtrlFSM::update_al_thrust_value, &fsm, _1));

    ros::Subscriber al_acceleration_sub =
        nh.subscribe<geometry_msgs::Vector3>("/al_controller/desired_acceleration",
                                             10,
                                             boost::bind(&PX4CtrlFSM::update_al_desired_acceleration, &fsm, _1));

    ros::Subscriber al_velocity_sub =
        nh.subscribe<geometry_msgs::Vector3>("/al_controller/desired_velocity",
                                             10,
